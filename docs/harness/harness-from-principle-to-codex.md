...
**Context rot（上下文腐烂）**：随着上下文窗口被填满，模型在推理和完成任务方面**变得更差**。上下文是稀缺资源，不是越多越好。这直接解释了为什么 OpenAI 团队把巨型 `AGENTS.md` 拆成分层文档（见 5.4 节）。
...
推理期择优是**临时**的选择压力——每次请求都要重新付 N 倍成本。10.7 提到的**拒绝采样微调（Rejection Sampling Fine-tuning, RFT）**正是把这份压力**固化进权重**的桥梁：
