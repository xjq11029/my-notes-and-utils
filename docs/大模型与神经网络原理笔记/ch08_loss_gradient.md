...
- 简单示例：预测值与标签逐项**相减再平方**（平方保证为正数；谁减谁无所谓）——这就是**均方误差 MSE（Mean Squared Error）**，多用于回归任务；**分类任务更常用交叉熵（cross-entropy）**，思路一样，都是把"差多少"压成一个 ≥0 的数字。
...
- 知道切线就知道**该往哪个方向走**：
  ...
  - "走多远"还不知道，但方向有了。
   ...
   ![演示：单参数情形——复杂未知曲线上的切线](<../2026-09-07%2012-15-43_doc/frames/frame_02-44-15.jpg>)
...
- 真实参数是 2 万多个维度——高维空间画不出来，但思想完全一样：**往最陡的方向走**。

   ![演示：3D 损失曲面——两个参数对应损失度](<../2026-09-07%2012-15-43_doc/frames/frame_02-46-45.jpg>)
