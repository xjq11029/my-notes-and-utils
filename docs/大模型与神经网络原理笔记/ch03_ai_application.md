...
> **时间轴** 00:44 – 01:12 ｜ 时长约 28 分钟

## 一、概念

**AI Application（AI 应用）** 是第三层：**封装模型服务**（正如模型服务封装裸模型，每一层封装下一层）。
...
> ⚠️ **两层界限其实不清晰**：很多概念会"流动"——**Skills** 最早是应用层玩出来的概念，后来模型服务商直接把它吸收进服务层（OpenAI 接口里已内置 Skill 能力）；**MCP、Tools** 同理。分工逻辑很简单：**服务商做了你就不用做，服务商没做你就必须做——总有一方要做**。
...
| 层 | 稳定性 | 说明 |
|---|---|---|
| 裸模型 | **非常稳定** | 调用方式（token 列表 → 概率分布）自 Transformer 以来未变；训练方式、性能、参数会变，但接口不变 |
...
> 注：这是课程的强判断，针对的是"**全量微调 + 两三个月验证**"这套传统玩法。业界仍在使用 **LoRA / PEFT** 这类低成本的参数高效微调（格式对齐、垂直领域适配、小模型蒸馏等），只是它已不是首选方案。

### 2.3 全课核心方法论：裸模型之上，只能玩两件事

> **论点：在裸模型基础上折腾的所有东西（模型服务 + AI 应用），只能玩两个东西——
> ① 给模型输入什么；② 如何处理模型的输出。**
...
**实战步骤**：
1. 准备代理服务器（老师现场让 AI 写了一个，仅实现"接收 → 转发 → 记录日志"）；配置 `.env`：端口 + 转发目标地址；
...
2. **Skill 就是提示词**：tools 数组里 Skill 工具的 description 明确告诉模型——什么是技能、何时该用（用户输入斜杠命令 `/xxx` 即指技能）、调用时要返回什么 JSON 格式。如此简单，毫无神秘。
   ...
   ![抓包证据：tools 里 Skill 的定义——告诉 AI 什么叫做技能](<../2026-09-07%2012-15-43_doc/frames/frame_01-02-24.jpg>)
...
3. **技能调用的完整流程**（见下方流程图）：
   ...
   ![抓包证据：SSE 流式输出"我将使用 web-design-guid"→即将进入 JSON 调用](<../2026-09-07%2012-15-43_doc/frames/frame_01-06-21.jpg>)

> **再次强调**：这节课讲的不是 Skill 本身，而是**分析方法**。以后遇到任何新概念（哪怕今天火的工具等你看视频时已经消失），都用"输入什么 / 输出什么 / 如何处理"三问拆解，几分钟搞清楚。

## 三、白板 / 演示步骤（按讲解顺序还原）

1. **画出三层封装关系**：模型服务封装裸模型；AI 应用封装模型服务；以 Skills 概念"从应用层流进服务层"说明两层界限模糊。
...
1. **三层封装链**：裸模型 ← 模型服务 ← AI 应用；越底层越稳定，越上层越不稳定。
...
- **越底层越稳定**：裸模型的调用方式（token 列表 → 概率分布）自 Transformer 以来未变；上层变动快不必焦虑。
- 微调没人玩的原因：验证贵 + 基础模型迭代碾压；玩就玩上两层。
- **核心方法论**：裸模型之上只能玩「输入什么 / 如何处理输出」两件事；新概念用三问拆解。
