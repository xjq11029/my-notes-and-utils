...
2. **系统提示词注入（system prompt injection，指服务商把系统提示词拼进请求）**：
   ...
   - **提示词的本质**：它不是模型层的概念，是**模型服务商层面的概念**。对裸模型而言，系统提示词、用户提示词、用户问题统统被转成**同一个 token 数组**，没有任何区别；"提示词"只是我们在含义上的分类。系统提示词与用户提示词冲突时：一般性内容听用户的，重要规则听系统的（训练数据与后训练对齐让模型对 system / user 这类 role 标记形成偏好）。
...
```js
while (true) {
  const prob = raw_model(input);            // ① 裸模型算概率分布
