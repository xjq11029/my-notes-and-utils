...
- **稳定性**：这套原理与知识体系**长期未变**——往上可追溯到 2017 年 Transformer 架构确立，2023 年 GPT-3.5 兴起后成为业界共识，至今没有本质变化。
...
| 语言 | 经验换算 |
|---|---|
| 中文 | 1 个字 ≈ **1.5 个 token**（有的字 1 个、有的 2~3 个，平均 1.5） |
...
两个关键特性：
1. **超限不报错，自动截断**：支持 1 万却传 2 万，前面的 1 万直接丢弃。
...
- **架构匹配**：用 HuggingFace 等库调用模型；用什么架构训练（当前主流 **Transformer**），就用什么架构调用；换架构需先做矩阵转换。这套模式属于**神经网络 / 深度学习**技术路线。
...
1. **画出黑盒**：白板中央先画一个框，写上 `Raw Model`——先不解释内部。

   ![白板 1：Raw Model 黑盒](<../2026-09-07%2012-15-43_doc/frames/frame_00-09.jpg>)

2. **补全数据流与分词换算**：左侧接 `Token List`、右侧接 `概率分布`；便签写下 `[2,3,4,5,1,8]`、`中文 1 字 = 1.5 token`、`英文 1 单词 = 1.3 token`。

   ![白板 2：Token List → Raw Model → 概率分布](<../2026-09-07%2012-15-43_doc/frames/frame_02-09.jpg>)

3. **填入概率分布示例**：输入"您好"对应的 token 列表，输出 `{15: 0.7, 288: 0.2, …}`。

   ![白板 3：概率分布示例](<../2026-09-07%2012-15-43_doc/frames/frame_03-09.jpg>)

4. **打开黑盒：接上权重矩阵**：在 Raw Model 下方用双向箭头连接"权重"框——"我们把它叫做权重矩阵"。

   ![白板 4：权重矩阵](<../2026-09-07%2012-15-43_doc/frames/frame_06-51.jpg>)

5. **控制台实测浮点误差**：切到浏览器 DevTools，现场输入 `0.1+0.2+0.3` 演示浮点数不满足结合律。

   ![演示：浮点误差](<../2026-09-07%2012-15-43_doc/frames/frame_12-03.jpg>)

6. **全图总结**：补上 GPU（双向箭头）与"训练/微调 + 语料 → 权重矩阵"，红框圈出全图收束本章。

   ![白板 5：全章总结图](<../2026-09-07%2012-15-43_doc/frames/frame_14-39.jpg>)
