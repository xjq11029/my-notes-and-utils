...
> **配图说明**：本课程示意图统一存放于根级 `assets/`（共 9 张，均为自绘，风格统一）。ch03 原有 2 张**真实抓包证据**截图（tools 定义、SSE 流式响应），因原始抓包目录 `2026-09-07 12-15-43_doc/` 已缺失，相关引用已移除。子课程 `coding-agent/` 自带 `assets/`（8 张**自绘示意图**，SVG 矢量源 + 2× PNG 导出，与根级配图同一风格；整理期拍摄的原始课件照片存档于其 `assets/_原始拍照/`）与 `逐字稿/`。请用 VSCode / Typora 打开本文件夹阅读（Mermaid 图会自动渲染）
...
| 子课程 | 文件 | 主题 | 素材 |
|---|---|---|---|
| 大模型与神经网络原理 | 根级 `ch01`–`ch10` | 裸模型 → 模型服务 → AI 应用 → 机器学习 → 神经网络 → 训练 | 根级 `assets/`（9 张自绘） |
| Coding Agent 原理与工具链 | 子目录 [`coding-agent/`](coding-agent/README.md)（`ch01`–`ch08`） | Agent 本质 → Tools → 会话 → 提示词 → 深度思考 → MCP → Skill/CLI → Sub Agent → 长期记忆 | `coding-agent/assets/`（8 张自绘示意图）+ `coding-agent/逐字稿/` |
