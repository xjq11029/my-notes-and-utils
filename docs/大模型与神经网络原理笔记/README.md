...
> 本目录收录**两门视频课程**的结构化学习笔记：**大模型与神经网络原理**（10 章，根级 `ch01`–`ch10`）与 **Coding Agent 原理与工具链**（8 章，子目录 [`coding-agent/`](coding-agent/README.md)）。
> **整理方式**：faster-whisper 语音转写 + 白板关键帧人工整理；口语已按书面语整理，忠实于原课内容
> **章节结构**：每章按「概念 → 原理推导 → 白板演示步骤 → 关键结论/公式 → 本节要点回顾」组织
> **配图说明**：本课程示意图统一存放于根级 `assets/`（共 10 张，均为自绘，风格统一；其中 `raw_model_panorama`（2026-09-22 按统一色板重画，同时修正副标题里的旧术语「裸模型」→「基础模型」）与 ch01 §2.4.1 新增的 `moe_two_counts` **保留了 SVG 矢量源**，其余 8 张仅有 PNG）。ch03 原有 2 张**真实抓包证据**截图（tools 定义、SSE 流式响应），因原始抓包目录 `2026-09-07 12-15-43_doc/` 已缺失，相关引用已移除。子课程 `coding-agent/` 自带 `assets/`（8 张**自绘示意图**，SVG 矢量源 + 2× PNG 导出，与根级配图同一风格；整理期拍摄的原始课件照片存档于其 `assets/_原始拍照/`）。请用 VSCode / Typora 打开本文件夹阅读（Mermaid 图会自动渲染）
...
| 子课程 | 文件 | 主题 | 素材 |
|---|---|---|---|
| 大模型与神经网络原理 | 根级 `ch01`–`ch10` | 基础模型 → 模型服务 → AI 应用 → 机器学习 → 神经网络 → 训练 | 根级 `assets/`（10 张自绘） |
| Coding Agent 原理与工具链 | 子目录 [`coding-agent/`](coding-agent/README.md)（`ch01`–`ch08`） | Agent 本质 → Tools → 会话 → 提示词 → 深度思考 → MCP → Skill/CLI → Sub Agent → 长期记忆 | `coding-agent/assets/`（8 张自绘示意图） |

---

## 大模型课程主线（一句话版）

**基础模型（token 进、概率分布出）→ 模型服务（API 三步封装）→ AI 应用（只能玩输入与输出）→ 要实现智能靠机器学习 → 机器学习靠神经网络 → 神经网络靠梯度下降 + 反向传播调参数 → 训练产物 = 权重文件 + 配置文件。**

## 大模型课程章节目录

| 章 | 文件 | 主题 |
|---|---|---|
| 1 | [ch01_raw_model.md](ch01_raw_model.md) | 基础模型 Raw Model：大模型的最底层真相（含课程外补充「稀疏 MoE 的两个口径」） |
| 2 | [ch02_model_service.md](ch02_model_service.md) | 模型服务 Model Service：API、定价与缓存（命中 / KV Cache）、采样参数 |
...
> **对外发布稿**：本课程前三章已整理为一篇面向掘金 / CSDN 的独立文章，见 [`articles/大模型原理/`](../../articles/大模型原理/llm-three-layers-from-raw-model-to-ai-application/README.md)。**改动 `ch01`–`ch03` 时需同步该发布稿**（同一事实的表述不允许分歧）。
...
| 术语 | 中文注释 | 首见 |
|---|---|---|
| Raw Model | 基础模型 / 原始模型——AI 体系最底层 | ch1 |
...
| MoE | Mixture of Experts，混合专家——把前馈网络（FFN）换成 N 个**专家（expert）** + 一个**路由器（router）**，每个 token 只放行得分最高的少数几个；"专家"并非分工，各自擅长什么由训练自己长出来 | ch1（课程外补充） |
| Total / Activated Parameters | 总参数（所有专家之和，决定文件体积与显存）/ 激活参数（每次真正参与计算的，决定算力与单 token 成本）——稀疏 MoE 让两者可差一个数量级 | ch1（课程外补充） |
...
| 术语 | 中文注释 | 首见 |
|---|---|---|
| Model Service | 模型服务——基础模型的二次封装，以 API 发布 | ch2 |
...
3. **AI 应用**在基础模型之上只能玩两件事：**给模型输入什么 + 如何处理模型输出**——万能概念分析法。
