...
> **配图说明**：本课程示意图统一存放于根级 `assets/`（共 9 张，均为自绘，风格统一）。ch03 原有 2 张**真实抓包证据**截图（tools 定义、SSE 流式响应），因原始抓包目录 `2026-09-07 12-15-43_doc/` 已缺失，相关引用已移除。子课程 `coding-agent/` 自带 `assets/`（8 张**自绘示意图**，SVG 矢量源 + 2× PNG 导出，与根级配图同一风格；整理期拍摄的原始课件照片存档于其 `assets/_原始拍照/`）。请用 VSCode / Typora 打开本文件夹阅读（Mermaid 图会自动渲染）
