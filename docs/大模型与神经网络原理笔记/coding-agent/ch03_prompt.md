...
| 工具 | 全局提示词 | 工程提示词 |
|---|---|---|
| Claude Code | 用户主目录下的 `CLAUDE.md` | 工程根目录的 `CLAUDE.md` |
| Codex | 用户主目录下的 `AGENTS.md` | 工程根目录的 `AGENTS.md` |
...
- **全局提示词很多时候可以不写**
- 讲师自己的全局提示词只有两句话，其中一句是：

  > 「不要自行决定，你不确定的话一定要问我。」

  （原因是遇到过很多次 Agent 自作主张的情况）
...
1. **看全局提示词**：打开用户主目录下的 `CLAUDE.md`（或 `AGENTS.md`）——讲师那份只有两句话
2. **看工程提示词**：打开工程根目录下的 `AGENTS.md`——描述工程在做什么 + 核心目录
3. **配置方式演示**：Codex 设置 → 个性化 → 配全局提示词；工程提示词则在工程根目录新建文件
4. **对照演示**：同一个需求（新增菜单），有 / 无工程提示词时模型的探索路径差异

![会话 → 提示词 → 深度思考：三级递进](assets/07_outline_conversation_prompt.png)

## 四、关键结论

1. **提示词分两级**：全局（用户主目录，所有工程）/ 工程（工程根目录，仅本工程）。
2. **具体文件**：Claude Code 用 `CLAUDE.md`，Codex 用 `AGENTS.md`；全局在用户主目录，工程在工程根目录。
3. **最佳实践②：提示词始终保持精简**。
...
- 位置：Claude Code → `CLAUDE.md`；Codex → `AGENTS.md`；全局在用户主目录，工程在工程根目录。
