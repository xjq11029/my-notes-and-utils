...
> 代码块内以 `%% 【补全】` 注释标出的部分，为原文档中 Mermaid 代码块被截断、依课件截图与课程内容补齐的内容。**标记必须写成 Mermaid 注释（`%%`）并独占一行**——写成普通文字会被解析器当成语法错误，导致整张图无法渲染。
...
> 说明：课件原图在「Agent->>MCP: 调用工具」处截断，以上标注 `%% 【补全】` 的部分依课件截图（存档于 `assets/_原始拍照/06_mcp_flow.jpg`）与课程讲解补齐。
