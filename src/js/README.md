...
| 模块 | 用途 | 第三方依赖（按需安装） |
|------|------|------------------------|
| `utils.js` | JS 常用工具：防抖 `debounce(fn, wait)`、日期格式化 `formatDate(date, locale)` | 无 |
...
- **按需引入**：各模块在文件顶层 `require` 其依赖，若未安装对应 npm 包则该模块 `require` 时报错——请按上表提前安装所需依赖，或仅引入无依赖的 `utils.js` / `errors.js`。
